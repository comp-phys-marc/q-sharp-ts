import { Paulis, OpenQASMCompatible, PossibleCompatibleScope, Function, Operation, Repeat, While, For, Condition, Conjugation, CCNOT, CNOT, H, I, M, Measure, R, R1, R1Frac, Reset, ResetAll, RFrac, Rx, Rxx, Ry, Ryy, Rz, Rzz, S, SWAP, T, X, Y, Z, Variable, Arr, Tuple, Range, IntType, BigIntType, DoubleType, BoolType, QubitType, Use } from './ast.js';
...
class Compiler {
     ...
     */
    constructor(ast, filePath = '', qubits, variables, variableTypes) {
                    ...
                    if (node instanceof Function || node instanceof Operation) {
                        ...
                        if (node.vals instanceof Arr) {
                            ...
                            for (let e = 1; e < node.vals.size; e++) {
                                if (node.vals[e].constructor.name == 'Int' && node.vals[e].val > max) {
                                    max = node.vals[e].val;
                                }
                                if (node.vals[e].constructor.name != ty) {
                                    if (node.vals[e] instanceof Variable) {
                                        let index = this.variables.indexOf(node.vals[e]);
